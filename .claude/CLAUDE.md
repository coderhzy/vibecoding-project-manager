...
**当用户请求开发任务时，你必须（MUST）执行以下流程：**

### 1. 立即创建今日日志
```
文件：.claude/daily/{YYYY}/{MM}/{DD}.md
动作：如果不存在则创建，存在则追加
```
...
**⚠️ 每个检查点必须调用 AskUserQuestion 工具，不能只是打印文字！**

```
检查点 1：需求分析 → AskUserQuestion("需求理解正确吗？", options: ["确认，继续", "需要修改"])
检查点 2：计划确认 → AskUserQuestion("任务分解合理吗？", options: ["确认，开始执行", "需要调整"])
检查点 3：执行完成 → AskUserQuestion("执行完成，验证吗？", options: ["验证", "继续开发"])
检查点 4：验证通过 → AskUserQuestion("验证通过，提交吗？", options: ["提交代码", "暂不提交"])
检查点 5：提交完成 → AskUserQuestion("继续还是结束？", options: ["继续开发", "结束会话"])
```

这样你会看到一个**交互式选择框**，必须点击才能继续。

### 4. 每个阶段完成后更新日志
- 需求分析 → 更新日志
- 生成 TODO → 更新日志 + TODO.md
- 执行每个任务 → 更新日志
- 验证完成 → 更新日志
- 提交代码 → 更新日志
